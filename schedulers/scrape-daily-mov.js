...
Bluebird.resolve()
  .then(async () => {
    ...
    const oldSlugs = JSON.parse(fs.readFileSync('schedulers/movie-urls.json', 'utf8'));

    const newSlugs = await getMovieUrls(movieType);

    const diffSlugs = _.difference(newSlugs, oldSlugs);

    fs.writeFileSync('schedulers/diff-movie-urls.json', JSON.stringify(diffSlugs, null, 2));
    ...
    fs.writeFileSync('schedulers/problematicUrls-movie.json', JSON.stringify(problematicUrls, null, 2));
    fs.writeFileSync('schedulers/movie-urls.json', JSON.stringify(newSlugs, null, 2));
  });
